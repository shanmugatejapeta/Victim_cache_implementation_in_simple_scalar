...
/* level 1 data cache, entry level data cache if (victim_cache)
...
/* Register stats for a victim cache and compute effective miss rate
   Effective demand misses = victim.misses - victim.writebacks
   Effective miss rate for L1+victim = effective_demand_misses / (l1.hits + l1.misses)
*/
static void
cache_reg_stats_victim(struct cache_t *vc, struct stat_sdb_t *sdb)
{
  char buf[512], buf1[512], *vname, *lname;
  /* victim cache name */
  ...
  /* assume L1 data cache name is cache_dl1 (global) */
  if (cache_dl1 && cache_dl1->name && cache_dl1->name[0])
    lname = cache_dl1->name;
  else
    lname = "<dl1>";

  /* Victim counters (expose raw counts) */
  sprintf(buf, "%s.accesses", vname);
  sprintf(buf1, "%s.hits + %s.misses", vname, vname);
  stat_reg_formula(sdb, buf, "total number of accesses (victim)", buf1, "%12.0f");

  sprintf(buf, "%s.hits", vname);
  stat_reg_counter(sdb, buf, "total number of hits (victim)", &vc->hits, 0, NULL);

  sprintf(buf, "%s.misses", vname);
  stat_reg_counter(sdb, buf, "total number of misses (victim, includes writebacks)", &vc->misses, 0, NULL);

  sprintf(buf, "%s.replacements", vname);
  stat_reg_counter(sdb, buf, "total number of replacements (victim)", &vc->replacements, 0, NULL);

  sprintf(buf, "%s.writebacks", vname);
  stat_reg_counter(sdb, buf, "total number of writebacks (victim)", &vc->writebacks, 0, NULL);

  sprintf(buf, "%s.invalidations", vname);
  stat_reg_counter(sdb, buf, "total number of invalidations (victim)", &vc->invalidations, 0, NULL);

  /* true demand misses at victim = victim.misses - victim.writebacks */
  sprintf(buf, "%s.true_misses", vname);
  sprintf(buf1, "(%s.misses - %s.writebacks)", vname, vname);
  stat_reg_formula(sdb, buf, "true demand misses at victim (exclude writeback lookups)", buf1, "%12.0f");

  /* victim true miss rate relative to victim accesses */
  sprintf(buf, "%s.true_miss_rate", vname);
  sprintf(buf1, "((%s.true_misses) / (%s.hits + %s.misses))", vname, vname, vname);
  stat_reg_formula(sdb, buf, "victim true miss rate (demand misses / victim accesses)", buf1, "%12.4f");
    /* Victim true accesses = victim.accesses - dl1.writebacks
     (exclude L1 writeback lookups) */
  sprintf(buf, "%s.true_accesses", vname);
  sprintf(buf1, "((%s.hits + %s.misses) - %s.writebacks)", vname, vname, lname);
  stat_reg_formula(sdb, buf,
      "total number of accesses (victim, excluding DL1 writebacks)", buf1, "%12.0f");
  /* Effective miss rate for L1+victim: numerator = victim.true_misses,
     denominator = l1.hits + l1.misses */
  sprintf(buf, "%s.effective_miss_rate", lname);
  sprintf(buf1, "((%s.true_misses) / (%s.hits + %s.misses))", vname, lname, lname);
  stat_reg_formula(sdb, buf, "effective miss rate (demand misses to memory / l1 accesses)", buf1, "%12.4f");
  
  /* Also register victim replacement and writeback rates relative to victim accesses */
  sprintf(buf, "%s.repl_rate", vname);
  sprintf(buf1, "(%s.replacements / (%s.hits + %s.misses))", vname, vname, vname);
  stat_reg_formula(sdb, buf, "victim replacement rate (i.e., repls/vacc)", buf1, "%12.4f");

  sprintf(buf, "%s.wb_rate", vname);
  sprintf(buf1, "(%s.writebacks / (%s.hits + %s.misses))", vname, vname, vname);
  stat_reg_formula(sdb, buf, "victim writeback rate (i.e., wbks/vacc)", buf1, "%12.4f");
}
...
{  
  if (cache_dl2)
  {
    /* Access next level of data cache hierarchy (DL2) */
  ...
  else
  {
    ...
    return 1;   /* access latency, ignored */
  }
}
...
dl1_access_fn(enum mem_cmd cmd,      /* access cmd, Read or Write */
              ...
              tick_t now)            /* time of access */
{  
  /* If victim cache exists, check it first */
  if (victim_cache)
  {
    unsigned int vlat = cache_access(victim_cache, cmd, baddr, NULL, bsize,
                                     /* now */ now, /* pudata */ NULL,
                                     /* repl addr */ NULL);

    /* If the victim cache hits, return its latency (skip L2) */
    if (vlat != 0)
      return vlat;
  }

  /* Otherwise, go to next level (victim miss → L2 or memory) */
  return victim_access_fn(cmd, baddr, bsize, blk, now);
}
...
/* victim cache options */
	     opt_reg_int(odb, "-victim:entries",
            "number of victim cache entries (0 disables victim cache)",
            &victim_entries, /* default */0, /* print */TRUE, NULL);
  ...
  if (!mystricmp(cache_dl1_opt, "none"))
      ...
      else
	{
	  ...
	  cache_dl2 = cache_create(name, nsets, bsize, /* balloc */FALSE,
				   /* usize */0, assoc, cache_char2policy(c),
				   dl2_access_fn, /* hit latency */6);
	  /* create victim cache between L1 and L2 if requested */
		if (victim_entries > 0)
  			{
  		  char vname[128];
   		 sprintf(vname, "victim_%d", victim_entries);
   		 /* fully associative victim cache: nsets=1, assoc=victim_entries */
   		 victim_cache = cache_create(vname,
                                /* nsets */ 1,
                                /* bsize */ bsize,
                                /* balloc */ FALSE,
                                /* usize */ 0,
                                /* assoc */ victim_entries,
                                cache_char2policy(c),
                                victim_access_fn, /* next-level */
                                /* hit latency */ 1);
  			}
	}
    }
...
sim_reg_stats(struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  if (victim_cache)
    cache_reg_stats_victim(victim_cache, sdb);
  if (itlb)
